...
public class Uniprogrammed{
	...
	private void runUniprogrammed(Boolean verbose){
		...
		while(finishedProcesses.size() < numberOfProcesses){
			...
			if(currProcesses == null && blockedProcesses.size() == 0){
				Collections.sort(readyQueue,Process.ArrivalComparator);
				if(readyQueue.size() > 0){
					currProcesses = readyQueue.remove(0);
				}
				if(currProcesses != null){
